...
class MascotaDAO:
    ...
    def insertar(self, mascota):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO mascotas (dueno_id, nombre, especie, raza, sexo, peso) VALUES (%s, %s, %s, %s, %s, %s) RETURNING id",
            (mascota.dueno_id, mascota.nombre, mascota.especie, mascota.raza, mascota.sexo, mascota.peso)
        ) 
        mascota.id = cursor.fetchone()["id"]
        conn.commit()
        conn.close()
    ...
    def actualizar(self, id, nombre=None, raza=None, peso=None):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE mascotas SET nombre = %s, raza = %s, peso = %s WHERE id = %s",
